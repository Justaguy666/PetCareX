...
export default function VetProfileView({ profile }: Props) {
            ...
            <div className="space-y-3">
                <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p className="font-medium text-foreground">{new Intl.NumberFormat('en-US', { style: 'currency', currency: 'VND', maximumFractionDigits: 0 }).format(profile.baseSalary)}</p>
                    </div>
                </div>
                                ...
                                <tbody>
                                    {profile.transferHistory.map((h: any, idx: number) => (
                                        <tr key={idx} className="border-t border-border">
                                            ...
                                            <td className="py-2 pr-4 text-muted-foreground">{h.date ? new Date(h.date).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: '2-digit' }) : '—'}</td>
                ...
                <div className="flex gap-2 pt-4">
                    <Button onClick={() => navigate('/vet/profile/edit')} className="bg-primary text-white">Edit Profile</Button>
                </div>
            </div>
        </Card>
    );
